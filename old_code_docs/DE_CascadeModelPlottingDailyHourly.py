...
in_df_720min_orig = os.path.join(main_dir, r'DE resampled 720min.csv')
...
in_df_360min_orig = os.path.join(main_dir, r' DE resampled 360min.csv')
...
in_df_180min_orig = os.path.join(main_dir, r'DE resampled 180min.csv')
...
def plotProbMonth(prob_df_file, cascade_level):
    ...
    out_figs_dir1 = os.path.join(out_dir, 'Seasonal effects on P01 %s'
                                 % cascade_level)
    if not os.path.exists(out_figs_dir1):
        os.mkdir(out_figs_dir1)
...
def probStation(prob_stn_df_file, cascade_level):
    ...
    out_figs_dir2 = os.path.join(out_dir, 'Prob P01 per stn %s'
                                 % cascade_level)
    if not os.path.exists(out_figs_dir2):
        os.mkdir(out_figs_dir2)
...
def volumeDependacyP01(in_df_files, in_param_file, cascade_level):
    ...
    out_figs_dir3 = os.path.join(out_dir, 'P01 unbounded model %s'
                                 % cascade_level)
    if not os.path.exists(out_figs_dir3):
        os.mkdir(out_figs_dir3)
    ...
    for df_file in in_df_files:
        ...
        for stn in stn_list:
            ...
            if fnmatch.fnmatch(df_file, '*.csv') and stn in df_file:
                ...
                # read df_file with coulms: R_vals, W_vals, W_vals(0_1),
                # L(teta)
                d = pd.read_csv(df_file, sep=df_sep, index_col=0)
...
#==============================================================================
                ...
                hist1, bins1 = np.histogram(np.log10
                                            (df_sim_vals['baseline rainfall %s' %
...
#==============================================================================
                ...
                hist2, bins2 = np.histogram(np.log10
...
#==============================================================================
            ...
            hist1, bins1 = np.histogram(np.log10(df_all['mean baseline'].values),
                                        bins=bins2,
                                        range=(-0.6, 1.5),
                                        normed=norme_it)
...
#==============================================================================
            ...
            hist2, bins1 = np.histogram(np.log10(df_all['mean unbounded'].values),
                                        bins=bins2,
                                        range=(-0.6, 1.5),
                                        normed=norme_it)
        ...
        #        try:
                ...
                for quartile in quartiles:
                    df_extremes_orig['orig extremes'] =\
                        df_sim_vals['orig vals'][df_sim_vals
                                                 ['orig vals'] >= quartile * max_orig]

                    df_extremes_baseline['baseline extremes'] =\
                        df_sim_vals['baseline rainfall %s' % cascade_level][df_sim_vals
                                                                            ['baseline rainfall %s' %
                                                                                cascade_level]
                                                                            >= quartile * max_baseline]

                    df_extremes_unbound['unbounded extremes'] =\
                        df_sim_vals['unbounded rainfall %s' % cascade_level][df_sim_vals
...
# call fct level one and two
L1_orig_sim = plotLorenzCurves(
    dfs_lorenz_L1,
    dfs_lorenz_L1_sim,
    cascade_level_1)
L2_orig_sim = plotLorenzCurves(
    dfs_lorenz_L2,
    dfs_lorenz_L2_sim,
    cascade_level_2)
L3_orig_sim = plotLorenzCurves(
    dfs_lorenz_L3,
    dfs_lorenz_L3_sim,
    cascade_level_3)
...
def distMaximums(in_df_orig_file, in_df_simu_files, cascade_level):
    ...
    if not os.path.exists(out_dir_dist):
        os.makedirs(out_dir_dist)
